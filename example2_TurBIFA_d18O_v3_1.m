...
%% Example 2 TurBIFA model
%
% This example uses d18O IFA data and constant SAR,BD and abundance 
%
% Note: just run the script to explore TurBIFA_d18O outputs
%
% --> TurbIFA_d18O mixing model builds on the SEAMUS software bioturbation 
% --> TurbIFA IFA noise stimation routines builds on the INFAUNAL (IFA 1σ) 
%     and QUANTIFA (IFA q-q space) algorithms.
...
Trace = 1;              % bioturbate Trace data (SAT + SSS)
run = 1;                % number of bioturbated synthetic sediment cores (default is 50, now 1 just to run faster)
abu = 0;                % constant abundance
anerr = 0.05;           % analytical uncertainty (1σ,‰) 
exerr = 0;              % extra gaussian noise from e.g., d18Osw-S relationship (1σ,‰) 
IFAtop_mage = 1950;     % mean age Coretop IFA (years, BP)
ystart_top = 2000;      % start year coretop IFA data timeslice (years, BP)
yend_top = 1900;        % end year  coretop IFA data timeslice (years, BP)
IFApast_mage = 5950;    % mean age downcore IFA (years, BP)
ystart_past = 6000;     % start year downcore IFA data timeslice (years, BP)
yend_past = 5900;       % end year downcore IFA data timeslice (years, BP)
dynamic = 0;            % constant SAR,BD and abundance
seas = 1:12;            % yearly specimens
sed_r = 10;             % sedimentation rate (cm/kyr)
BD = 15;                % bioturbation depth (cm)
num_q = 50;             % numer of quantiles
mc = 1000;              % repeat "picking" mc times
clim_start = -40;       % relative to 1950 CE
months_kyrs = 12*1000;

%% 1.1 Load pseudo-d18O TraCE-21ka time series

Trace_full=load('data_examplesTurBIFA.mat','Traced18O_ex2');
Trace_full=Trace_full.Traced18O_ex2;

%% 1.2 Load IFA data 

X=load('data_examplesTurBIFA.mat','LH_data2');
X=X.LH_data2;
Y=load('data_examplesTurBIFA.mat','Past_data2');
Y=Y.Past_data2;

clim_startm= (clim_start*12);  % start month climate time series, relative to 1950 CE

%% 2 Prepare d18O time series before bioturbation simulation
