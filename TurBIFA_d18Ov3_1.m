...
%% About
%
% TurbIFA_d18O performs a bioturbation simulation, propagates the noise in
% d18O IFA datasets and asses the statistical significance of IFA
% reconstructions
% 
% --> TurbIFA_d18O mixing model builds on the SEAMUS software bioturbation 
% --> TurbIFA_d18O builds on the INFAUNAL and QUANTIFA algorithms
%     to estimate the noise in the std and shape (q-q space) 
%     of the IFA distribution, respectively.
% --> TurbIFA_d18O d18Osw-sea level correction builds on the Paleo-Solver v2.1 
%     algorithm
%     
% Source SEAMUS:
...
%
% Source PSU-Solver
% https://www.mathworks.com/matlabcentral/fileexchange/59565-paleo-seawater-uncertainty-solver
...
% Before run !!!:
% 
% ------ X =  CoreTop  IFA distribution (mean not removed) 
% ------ Y =  Downcore IFA distribution (mean not removed) 
% ------ Trace= 1 The program will automatically load SAT and SSS Trace data (default)
% ------ Trace= 0 The user will load other d18O time series named
%                 "clim_full" and indicated the start year  "clim_start" (youngest age)
% ------ sl_slope = slope (‰/m) for d18Osw-sea level correction (0.008 default)
...
%                to an user defined scale parameter e.g., 10. As the scale parameter increases, smaller changes
...
%              biotubation depth and/or abundance are not constant for
%              probability heatmap output
...
% ------ analytical uncertainty (1σ,‰)
% ------ extra gaussian uncertainty = from e.g., d18Osw-S relationship (1σ,‰) 
% ------ d18O-S relationship  = Select from LeGrande & Schmidt (2006)  or
%                               define own equation
...
%
% 3. Summary table of the uncertainties of the IFA d18O distribution, 
...
%                           (quantiles orderer from lighter to heavier d18O) 
...
%% 1.1 Before run !
Trace= 1;           % bioturbation simulation using SAT and SSS Trace data. 
                    % Set Trace= 0 to load other predicted-d18O time series
                    % "clim_full"
...
sl_slope = 0.008;   % sea level slope for d18Osw correction (‰/m, Schrang et al., 2002).

% X=                % CoreTop  IFA dataset
% Y=                % Downcore IFA dataset
...
x = inputdlg({'number quantiles:', 'analytical 1σ (‰):', 'extra gaussian noise 1σ (‰):' ...
...
anerr=str2double(x{2,1});                % analytical noise (1σ,‰)
exerr=str2double(x{3,1});                % extra gaussian noise (1σ,‰) If not required, just input 0
...
% d18Osw-S relationship (the list follows 'datafor_d18Osw.LegrSmit' order)
    
list = {'LeGrande & Schmidt (2006) Artic Ocean= 0.48*S - 16.82',...
        'LeGrande & Schmidt (2006) Baffin Bay= 0.33*S - 11.82',...
        'LeGrande & Schmidt (2006) Baltic Seas= 0.28*S - 8.73',...
        'LeGrande & Schmidt (2006) GIN Seas = 0.60*S - 20.71',...
        'LeGrande & Schmidt (2006) Hudson Bay= 0.42*S - 16.05',...
        'LeGrande & Schmidt (2006) Indian Ocean=0.16*S - 5.31',...
        'LeGrande & Schmidt (2006) Labrador Sea= 0.94*S - 32.45',...
        'LeGrande & Schmidt (2006) Mediterranean= 0.28*S - 9.24',...
        'LeGrande & Schmidt (2006) North Atlantic = 0.55*S - 18.98',... 
        'LeGrande & Schmidt (2006) North Pacific = 0.44*S - 15.13',...
        'LeGrande & Schmidt (2006) Tropical Atlantic= 0.15*S - 4.61',...
        'LeGrande & Schmidt (2006) Tropical Pacific Ocean= 0.27*S - 8.88',...
        'LeGrande & Schmidt (2006) Red Sea/Persian Gulf= 0.31*S - 16.81',...
        'LeGrande & Schmidt (2006) South Atlantic = 0.51*S - 17.40',...
        'LeGrande & Schmidt (2006) South Pacific = 0.45*S - 15.29',...
        'LeGrande & Schmidt (2006) Southern Ocean= 0.24*S - 8.45',...
        'Other'};

[swS,~] = listdlg('ListString',list,'SelectionMode','single','ListSize',[300,200],...
                       'Name','d18Osw-S relationship');

if swS==17 % other d18OswS slope and intercept
    prompt = {'Slope','Intercept'};
    dlgtitle = 'Enter d18Osw-S relationship';
    definput = {'0.36','-9.44'};
    swSuserparams = inputdlg(prompt,dlgtitle,[1,10],definput);
end
...
%% 2. Load Trace data and calculate predicted-d18O 

if Trace==1   % Load monthly SAT and decadal SSS Trace data

    fig = uifigure;
    d = uiprogressdlg(fig,'Title','Loading TraCE data',...
        'Indeterminate','on');
  
    load("Trace_coordinates.mat",'TraceSAT_lat','TraceSAT_lon','TLAT_SSS','TLON_SSS')                
    dataST = matfile('Trace_SAT.mat');        % .mat file TraCE-21ka SAT time series
    dataSSS = matfile('SSS_Trace.mat');       % .mat file TraCE-21ka SSS time series
    load('datafor_d18Osw.mat','sea_level');   %  0-23 kyr sea level curve (Lambeck et al. 2014)
    load('datafor_d18Osw.mat','LegrSmit');    % Legrande & Smitch (2006) d18Osw-S slopes and intercepts                   
    ...
    Trace_T=dataST.Trace_SAT(:,T_lat,T_lon)-273.15; % K to ºC

    % Prepare SSS coordinates
    % lets find core site position inside TraCE SSS product (bidimensional lat and lon)
    
    lonpos=find(abs(lon-TLON_SSS)<3.6);  % cutoff = TraCE longitudinal resolution
    latpos=find(abs(lat-TLAT_SSS)<20);   % generous cutoff
    [val,~]=intersect(lonpos,latpos);
    [~,pos]=min(abs(TLON_SSS(val) - lon) + abs(TLAT_SSS(val)- lat)); % find nearest point to core location
    gridv=val(pos); 
    l=reshape(1:size(TLAT_SSS,2)*size(TLAT_SSS,1),size(TLAT_SSS,1),size(TLAT_SSS,2));
    [S_lat,S_lon]=find(l==gridv);

    % get TraceSSS data at core location and rescale to monthly resolution
    dec_months=10*12;  % nº months per decade
    Trace_S=squeeze(dataSSS.SSS_Trace(:,S_lat,S_lon));
    Trace_S=reshape(repmat(Trace_S,1,dec_months)',[],1); % rescaled 
    
    %% 2.1 d18O Forward model 
    
    if swS <= 16 % from LeGrande & Schmidt (2006)
        local_d18Osw= LegrSmit(swS,1)*Trace_S + LegrSmit(swS,2);
    else          % other source  
        local_d18Osw= sscanf(swSuserparams{1, 1},'%f')'*Trace_S + sscanf(swSuserparams{2, 1},'%f');
    end

    cum_month=(clim_startm:1:length(Trace_T)-1 + clim_startm)/12; % monthly time steps
    agesl= zeros(numel(sea_level(:,1))+1,1); % sl age (kyr)
    sl= zeros(numel(sea_level(:,1))+1,1);    % sl (m)
    agesl(1)=clim_start/1000;                % set sl age(1) = 1990 CE, in kyr
    agesl(2:end)=sea_level(:,1);            
    sl(2:end)=sea_level(:,2);

    % d18Osw-sea level correction
    SL = flip(interp1(agesl*1000,sl,cum_month));  % interpolate slevel monthly scale
    d18Osw = local_d18Osw - sl_slope*SL';         % d18Osw slevel correction
    
    % predicted-d18O after apply HL Bemis et al. (1998) and Hut (1987) correction
    Trace_full= (d18Osw-0.27) + (14.9-Trace_T)/4.8; % reused from INFAUNAL algorithm (see souce above)
...
%% 2.2 Prepare d18O time series before bioturbation simulation
...
rl = refline(1,0); 
rl.Color = 'k';
xlabel('CoreTop IFA  Quantiles  (‰, mean removed)');
ylabel('Downcore IFA Quantiles (‰, mean removed)');
...
rl = refline(1,0); 
rl.Color = 'k';
xlabel('Coretop (‰, mean removed)')
ylabel('Downcore (‰, mean removed)')
